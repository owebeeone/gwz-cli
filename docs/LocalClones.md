...
git-produced states. That file is not your work, but `gwz local dispose`
counts it as `dirty` (see [Retire](#4-retire)).
...
status: Ok
state: completed
merge: merge_op_80300_1788669327332_0001 (closed)
record: v1 (archived)
terminal outcome: completed
acceptance: supported-persisted
participants: total 1; fast-forwarded 1
publication: complete
participants:
  services/api (mem_api)  fast-forwarded
    source: refs/gwz/local-imports/xfer_c6de6dd3b1e248f0ecfaca9eb8f3dc42 @ 395c9b6f88fdca9159ab58071c1c309ed761acfb
    recorded: branch main; before b62fc6260442c2c0eb7cbae4227894550ef1904e; result 395c9b6f88fdca9159ab58071c1c309ed761acfb
    live: commit 395c9b6f88fdca9159ab58071c1c309ed761acfb
    recovery: continue unknown; abort unknown
```
...
- **The root is not a participant unless you select it.** `gwz merge --remote
  A` merges the members and leaves the root repository alone, as every merge
  does. The lane's root commits — every `gwz commit` made there produced one
  — come across only with an explicit root selection:
...
member. Immediately after the member-only merge above, the lane's members
are preserved but its root repository is not:
