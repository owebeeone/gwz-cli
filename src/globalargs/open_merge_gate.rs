...
pub(super) fn open_merge_gate_request(
    ...
    let (meta, command) = match request {
        ...
        // The Claude Code commands never enter the gate: the hook drives
        // core's own family verbs, which take it themselves (D1).
        CliRequest::Hook(_) | CliRequest::ClaudeCodeSetup(_) => return None,
    };
    Some((meta, command))
}
